...
import NamePattern, CustomStatePanel
...
class MakeAToon:
    ...
    def dismiss(self):
        ...
        base.camLens.setFov(ToontownGlobals.DefaultCameraFov, ToontownGlobals.DefaultCameraFov / 1.3)
        base.cam.setPos(0)
    ...
    def finalize(self):
        ...
        if config.GetBool('want-custom-stats', True):
            self.csp = CustomStatePanel.CustomStatePanel()
            base.cr.acceptOnce(self.csp.doneEvent, self.__handleCsp)
    ...
    def __handleCsp(self, hood, trackChoice):
        self.csp.destroy()
        self.csp.removeNode()
        self.__createAvatar(not self.skipTutorial, hood, trackChoice)
            
    def __createAvatar(self, tf, hood=0, trackChoice=[-1, -1]):
        ...
        base.cr.csm.sendCreateAvatar(self.toon.style, nameData, self.slot)
